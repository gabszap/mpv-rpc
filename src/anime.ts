...
// Circuit Breaker - prevents hammering the API when it's failing
const circuitBreaker = {
    failures: 0,
    lastFailure: 0,
    threshold: 5,           // Open circuit after 5 consecutive failures
    cooldown: 30000,        // Wait 30 seconds before retrying
    isOpen(): boolean {
    ...
    recordFailure(): void {
        ...
        if (this.failures >= this.threshold) {
            console.log(`[Anime] Circuit breaker OPEN - API failing, waiting ${this.cooldown / 1000}s before retry`);
        }
    },
    recordSuccess(): void {
        if (this.failures > 0) {
            this.failures = 0;
            console.log("[Anime] Circuit breaker CLOSED - API recovered");
        }
    },
    reset(): void {
        this.failures = 0;
        console.log("[Anime] Circuit breaker RESET - Retrying API");
    },
...
/**
 * Manage log file size (truncate if > 5MB)
 */
function manageLogFile(): void {
    try {
        const logPath = getLogPath();
        if (fs.existsSync(logPath)) {
            ...
            if (stats.size > 5 * 1024 * 1024) {
                console.log("[Anime] Log file too large, cleaning up...");
                fs.writeFileSync(logPath, ""); // Clear file
            }
        }
    } catch (e) {
        console.error("[Anime] Error managing log file:", e);
    }
}
...
/**
 * Make a request to Jikan API with rate limiting and circuit breaker
 */
async function jikanRequest(endpoint: string, params?: Record<string, any>, retryCount = 0): Promise<any> {
    // Circuit breaker check
    if (circuitBreaker.isOpen()) {
        const remaining = Math.ceil(circuitBreaker.getRemainingCooldown() / 1000);
        logApiCall(endpoint, params, "CIRCUIT_OPEN", `waiting ${remaining}s`);
        throw new Error(`Circuit breaker open, retry in ${remaining}s`);
    }
